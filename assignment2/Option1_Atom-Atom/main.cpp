...
int bondValue(BondType bt) {
	...
	case BondType::Aromatic: throw mod::InputError("Aromatic bonds are not supported.");
	}
}
...
std::vector<std::shared_ptr<mod::rule::Rule> > doStuff(const std::vector<std::shared_ptr<mod::graph::Graph> > &educts,
	...
	const auto makeUnion = [](const auto &gs) {
		LUG lug;
		for(const auto &g : gs) lug.push_back(&g->getGraph().getLabelledGraph());
		return lug;
	};
		...
		for(const auto v : asRange(vertices(gEduct))) {
			for(const auto j : asRange(vertices(gProduct))) {
				if(pMolEduct[v] == pMolProduct[j]){
					EtoP.insert(pair<int, int>(getVertexId(v, gEduct), getVertexId(j, gProduct)));
					break;
				}
			}
			break;
	...
	for(auto &r : rulesInternal) rules.push_back(mod::rule::Rule::makeRule(std::move(r)));
	return rules;
}
